...
const blogPosts1 = [
  ...
  },
  {
    ...
    uk: {
      ...
      lead: "В інтернеті десятки порад, як «пройти поліграф». Більшість із них про класичний прилад із датчиками і про 1990-ті. Розберемо по черзі, що з цього досі працює. Коротко: ці прийоми розраховані на класичний поліграф, а не на тест за рухами очей.",
      sections: [
        { h2: "Поради з інтернету і що з ними не так", paragraphs: ["Усі відомі способи протидії розраховані на класичний поліграф: він вимірює дихання, тиск і потовиділення, і ці показники можна змінити свідомо."], list: ["Кнопка в черевику, напруження м'язів: створює штучну реакцію на контрольні питання. Досвідчений поліграфолог бачить це на графіку, а EyeDetect не вимірює тіло взагалі.", "Заспокійливі або алкоголь: притупляють реакції рівномірно, і тест визнається недійсним. На EyeDetect людина у стані сп'яніння до тесту не допускається.", "Рахування в умі, згадування неприємного: розраховане на поліграф, бо змінює дихання і тиск. Зіниця від розумового навантаження теж розширюється, але EyeDetect оцінює не одну реакцію, а різницю між типами тверджень разом із часом відповідей і помилками.", "«Повірити у власну брехню»: гарно звучить, але мозок усе одно витрачає більше зусиль на неправдиву відповідь. Саме це зусилля і фіксується."] },
        { h2: "Чому очі не обманюють", paragraphs: ["Зіниця розширюється на частки міліметра, коли мозок працює інтенсивніше. Свідомо керувати цим неможливо, як неможливо наказати серцю не битися. EyeDetect також фіксує швидкість читання, повернення до слів, час відповіді і помилки. Алгоритм зважує десятки таких показників, і людина жодного з них не контролює.", "Є ще одна деталь. Твердження на екрані повторюються багато разів у різних формулюваннях. Обманути один раз можна. Обманювати однаково 60 разів поспіль значно важче."] },
    ...
    ru: {
      ...
      sections: [
        ...
        { h2: "Что действительно влияет на результат", paragraphs: ["Не хитрости, а состояние: недосып, сильное волнение, плохо сформулированные вопросы. Поэтому перед тестом мы объясняем каждый вопрос, даём тренировочный тест и просим выспаться. Неопределённый результат чаще всего бывает из-за усталости или волнения. Тогда мы проводим повторный тест за наш счёт."] },
      ],
      faq: [
    ...
    en: {
      ...
      sections: [
        { h2: "Tips from the internet and what is wrong with them", paragraphs: ["All the known countermeasures are designed for the classic polygraph: it measures breathing, blood pressure and sweating, and these can be changed deliberately."], list: ["A tack in the shoe, tensing muscles: creates an artificial reaction to control questions. An experienced polygraph examiner sees it on the chart, and EyeDetect does not measure the body at all.", "Sedatives or alcohol: blunt reactions evenly, and the test is declared invalid. On EyeDetect, a person under the influence is not admitted to the test.", "Counting in your head, recalling something unpleasant: aimed at the polygraph, because it changes breathing and blood pressure. Mental effort does dilate the pupil too, but EyeDetect does not assess a single reaction: it compares types of statements together with response times and errors.", "“Believing your own lie”: sounds good, but the brain still spends more effort on a false answer. That effort is exactly what is recorded."] },
        { h2: "Why the eyes do not lie", paragraphs: ["The pupil dilates by fractions of a millimeter when the brain works harder. This cannot be controlled consciously, just as you cannot order your heart to stop beating. EyeDetect also records reading speed, re-reading of words, response time and errors. The algorithm weighs dozens of such measures, and the person controls none of them.", "There is one more detail. The statements on the screen are repeated many times in different wordings. You can lie once; lying consistently 60 times in a row is much harder."] },
  ...
  },
  {
    ...
    uk: {
      ...
      sections: [
        ...
        { h2: "Для онлайн-тесту VerifEye", paragraphs: ["Тиха кімната, рівне світло без вікна за спиною, телефон на підставці на рівні очей, авіарежим з Wi-Fi, 30 вільних хвилин. Окуляри з сильними лінзами можуть заважати камері, тоді краще тест у кабінеті."] },
      ],
      faq: [
    ...
    ru: {
      ...
      sections: [
        ...
        { h2: "Для онлайн-теста VerifEye", paragraphs: ["Тихая комната, ровный свет без окна за спиной, телефон на подставке на уровне глаз, авиарежим с Wi-Fi, 30 свободных минут. Очки с сильными линзами могут мешать камере, тогда лучше тест в кабинете."] },
      ],
      faq: [
    ...
    en: {
      ...
      sections: [
        ...
        { h2: "For the VerifEye online test", paragraphs: ["A quiet room, even lighting with no window behind you, the phone on a stand at eye level, airplane mode with Wi-Fi on, 30 free minutes. Glasses with strong lenses can interfere with the camera; in that case the test in our Lviv office is the better option."] },
      ],
      faq: [
  ...
  },
  {
    ...
    uk: {
      ...
      sections: [
        ...
        { h2: "Чому ми кажемо це чесно", paragraphs: ["Ми не видаємо «висновок для суду»: результат EyeDetect не є судовою експертизою. Для суду ми пропонуємо супровідний лист спеціаліста до PDF-звіту: що перевіряли, як проходив тест і що означає результат. Чи прийняти ці документи, вирішує суд у кожному окремому випадку. Тест дає оцінку від 1 до 99 по кожному питанню і показує, яка версія подій витримує перевірку. Що з цим робити далі, вирішуєте ви, іноді з адвокатом."] },
    ...
    ru: {
      ...
      sections: [
        ...
        { h2: "Почему мы говорим это честно", paragraphs: ["Мы не выдаём «заключение для суда»: результат EyeDetect не является судебной экспертизой. Для суда мы предлагаем сопроводительное письмо специалиста к PDF-отчёту: что проверяли, как проходил тест и что означает результат. Принимать ли эти документы, решает суд в каждом отдельном случае. Тест даёт оценку от 1 до 99 по каждому вопросу и показывает, какая версия событий выдерживает проверку. Что с этим делать дальше, решаете вы, иногда с адвокатом."] },
    ...
    en: {
      ...
      sections: [
        ...
        { h2: "Why we say this openly", paragraphs: ["We do not issue a “court opinion”: an EyeDetect result is not a forensic expert examination. For court we offer a cover letter from the examiner to accompany the PDF report: what was tested, how the test went, and what the result means. Whether to accept these documents is decided by the court in each individual case. The test gives a score from 1 to 99 on each question and shows which version of events holds up under scrutiny. What to do with that next is up to you, sometimes together with a lawyer."] },
